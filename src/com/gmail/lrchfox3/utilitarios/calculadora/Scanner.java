...
class Scanner
{
   ...
   {  StringBuffer buffer = new StringBuffer();
      ...
      {  char c = text.charAt(i);
         if (c == '\t')
         {  int skip = 8 - col%8;
            for (int j = 0; j < skip; j++) buffer.append(' ');
            col = col + skip;
         }
         else
         {  buffer.append(c);
   ...
   {  PreToken num;
      ...
      {  j++;
         ...
         else num = new PreToken(i, j, Symbol.BAD_NUMBER);
      }
      else num = new PreToken(i, j, Symbol.NUMBER);
      return num;
   }
   ...
   {  String source = program.source;
      ...
      {  PreToken tok = toks[i];
         ...
         if (tok.start == source.length()) spelling = "";
         else spelling = source.substring(tok.start, tok.end);
         ...
         {  int r = syms.size();
            ...
            else syms.add(new Symbol(spelling, tok.kind, 0.0));
            table.put(spelling, new Integer(r));
            tokens[i] = new Token(tok.start, r);
         }
         else tokens[i] = new Token(tok.start, ref.intValue());
      }
