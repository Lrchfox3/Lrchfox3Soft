...
{  Rule[] rules;
   ...
   {  int ndefs = grammar.length;
      ...
      {  definitions[i] = scanDef(grammar[i]);
         ...
         if (i == 0) indexes[i] = 0;
         else indexes[i] = indexes[i-1] + sizes[i-1];
      }
      for (int i = 0; i < primNames.length; i++)
      {  names[ndefs+i] = primNames[i];
         sizes[ndefs+i] = 1;
         if (i == 0) indexes[ndefs+i] = indexes[ndefs-1] + sizes[ndefs-1];
         else indexes[ndefs+i] = indexes[ndefs+i-1] + 1;
      }
   ...
   {  int nalts;
      ...
      nalts = 1;
      for (int i = 2; i < def.length; i++) if (def[i].equals("|")) nalts++;
      ...
      {  int[] seqIndexes = new int[altEnds[i] - altStarts[i]];
         ...
         if (seqIndexes.length == 1) altIndexes[i] = seqIndexes[0];
         else parseSeq(seqIndexes, altIndexes[i]);
      }
   ...
   private void parseSeq(int[] items, int r)
   {
      ...
      {  if (i < items.length - 2) rules[r+i] = new Rule.Then(items[i], r+i+1);
         else rules[r+i] = new Rule.Then(items[i], items[i+1]);
      }
   }
   ...
   {  for (int i = 0; i < alts.length - 1; i++)
      {  if (i < alts.length - 2) rules[r+i] = new Rule.Or(alts[i], r+i+1);
         else rules[r+i] = new Rule.Or(alts[i], alts[i+1]);
      }
   }
